...
const sidebarLinks: SidebarLink[] = [
  ...
  { title: "Resume", href: "https://jasreenmanderr.github.io/resume.pdf" },
        ...
        <div className="py-20 overflow-y-auto text-center">
          <ul className="flex flex-col items-center space-y-6">
            {sidebarLinks.map(({ title, href, isIcon, iconPath }) => (
              <div onClick={() => handleInternalNavigation(title)}>
                <li key={title} className="flex items-center">
                  <a
                    href={href != "/about" ? href : undefined}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 p-2 transition-transform hover:scale-105 cursor-pointer"
                  >
                    {isIcon ? (
